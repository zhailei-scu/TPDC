object SmoothingForm: TSmoothingForm
  Left = 618
  Top = 311
  AutoSize = True
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'SmoothSet'
  ClientHeight = 193
  ClientWidth = 259
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object RzBitBtnOK: TRzBitBtn
    Left = 6
    Top = 168
    Width = 67
    TabOrder = 0
    OnClick = RzBitBtnOKClick
    Kind = bkOK
  end
  object RzBitBtnClose: TRzBitBtn
    Left = 86
    Top = 168
    Width = 67
    TabOrder = 1
    OnClick = RzBitBtnCloseClick
    Kind = bkClose
  end
  object RzRadioGroupGauss: TRzRadioGroup
    Left = 0
    Top = 0
    Width = 259
    Height = 49
    Align = alTop
    Caption = 'Gauss Smooth'
    Color = clSkyBlue
    GroupStyle = gsBanner
    ItemFrameColor = clBtnFace
    TabOrder = 2
    object Gauss3Orders: TRzRadioButton
      Left = 16
      Top = 24
      Width = 65
      Height = 17
      Caption = '3 orders'
      TabOrder = 0
    end
    object Gauss5Orders: TRzRadioButton
      Left = 136
      Top = 24
      Width = 81
      Height = 17
      Caption = '5 orders'
      TabOrder = 1
    end
  end
  object RzRadioGroupMedium: TRzRadioGroup
    Left = 0
    Top = 49
    Width = 259
    Height = 49
    Align = alTop
    Caption = 'Medium Smooth'
    Color = clSkyBlue
    GroupStyle = gsBanner
    TabOrder = 3
    object Medium3Orders: TRzRadioButton
      Left = 16
      Top = 24
      Width = 65
      Height = 17
      Caption = '3 orders'
      TabOrder = 0
    end
    object Medium5Orders: TRzRadioButton
      Left = 136
      Top = 24
      Width = 81
      Height = 17
      Caption = '5 orders'
      TabOrder = 1
    end
  end
  object RzRadioGroupAverage: TRzRadioGroup
    Left = 0
    Top = 98
    Width = 259
    Height = 49
    Align = alTop
    Caption = 'Average Smooth'
    Color = clSkyBlue
    GroupStyle = gsBanner
    TabOrder = 4
    object Average3Orders: TRzRadioButton
      Left = 16
      Top = 24
      Width = 65
      Height = 17
      Caption = '3 orders'
      TabOrder = 0
    end
    object Average5Orders: TRzRadioButton
      Left = 136
      Top = 24
      Width = 81
      Height = 17
      Caption = '5 orders'
      TabOrder = 1
    end
  end
  object RzBitBtnReset: TRzBitBtn
    Left = 176
    Top = 168
    Caption = ' ReSet'
    TabOrder = 5
    OnClick = RzBitBtnResetClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
end
